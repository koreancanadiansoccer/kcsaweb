        ...
        <Box display="flex" justifyContent="start" alignItems="start">
          <Box display="flex" flexDirection="column" justifyContent="start" alignItems="center" minWidth={275}>
            ...
            <Box display="flex" justifyContent="center" alignItems="center" mb={5}>
              <LeagueSelect title="OPEN" selected={league === LeagueAgeType.OPEN} onClick={() => setLeague(LeagueAgeType.OPEN)} />

              <LeagueSelect title="SENIOR" selected={league === LeagueAgeType.SENIOR} onClick={() => setLeague(LeagueAgeType.SENIOR)} />
            </Box>

            {/* League table */}
            {league === LeagueAgeType.OPEN && (
              <motion.div initial={{ opacity: 0, x: -50, y: -50 }} animate={{ opacity: 1, x: 0, y: 0 }} transition={{ delay: 0.2 }}>
                {/* League table */}
                <LeagueTable tableType={TableType.STANDING} leagueType={LeagueAgeType.OPEN} />
                {/* Score table */}
                <Box mt={5}>
                  <LeagueTable tableType={TableType.SCORER} leagueType={LeagueAgeType.OPEN} />
                </Box>
              </motion.div>
            )}
            ...
            {league === LeagueAgeType.SENIOR && (
              <motion.div initial={{ opacity: 0, x: -50, y: -50 }} animate={{ opacity: 1, x: 0, y: 0 }} transition={{ delay: 0.2 }}>
                {/* League table */}
                <LeagueTable tableType={TableType.STANDING} leagueType={LeagueAgeType.SENIOR} />

                {/* Score table */}
                <Box mt={5}>
                  <LeagueTable tableType={TableType.SCORER} leagueType={LeagueAgeType.SENIOR} />
                </Box>
              </motion.div>
            )}
          </Box>
